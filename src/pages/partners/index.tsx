...
const partnerData = [
	{
		name: 'Luxvps',
		image: '/partners/luxvps.png',
		link: 'https://luxvps.net/',
		description:
	...
	{
		name: 'Burst Creators',
		image: '/partners/burstcreators.png',
		link: 'https://burstcreators.com/',
		description:
			'With Burst you only pay for actual views. Never overspend on influencer marketing again! Empower your brand with Burst—where impactful trends and measurable engagement drive your influencer campaigns. Pay for performance, not promises.',
	},
]
			...
			>
				<section className="">
					<div
						...
						className="max-w-2xl mx-auto rounded-3xl shadow-clean flex flex-col gap-6 shadow-main/10 border border-gray-600/10 p-10"
					>
						{partnerData.map((partner) => (
							<PartnerCards key={partner.name} Obj={partner} />
						))}
					</div>
				</section>
			</CurvePageIntro>
...
const PartnerCards = (props: any) => {
	return (
		<Link
			target="_blank"
			href={props.Obj.link}
			className="bg-white flex flex-col gap-3 p-10 text-gray-800 border border-gray-600/10 rounded-2xl"
		>
			<img
				className="w-14 h-14 mb-4 object-cover rounded-full border border-gray-600/10"
				src={props.Obj.image}
			></img>
